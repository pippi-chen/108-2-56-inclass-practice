...
##4.4 relational operators(T/F vector)
...
#如果覺得以上levels的排序是對的，則使用下方的order
example$allowance <- 
...
可選出「30_39歲以上受害者」的「要/不要」向量：(記得要相將此類別資料變為可排序的資料)
```{r}
...
R還有一種特別的缺失資料NaN (即not a number) -> 只會在數值出現:
出現在沒有定義的數學運算上，如：
```{r}
0/0
log(-10)
...
以下資料為新北市垃圾車路線
```{r}
jsonlite::fromJSON("https://data.ntpc.gov.tw/api/datasets/EDC3AD26-8AE7-4916-A00B-BC6048D19BF8/json") -> garbageRoutes
```
...
#字元規律描述regular expression -> 找的字必須由a字元開頭的元素
stringr::str_detect(fruit, "^a")
