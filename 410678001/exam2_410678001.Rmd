...
```{r ans13}
table(tpe_highSchools)
regex_hs1st <- "[臺台]?北市立?第一女?(子高級)?中?(學)?"
class(regex_hs1st)
length(regex_hs1st)
...
```{r ans14}
str_extract(tpe_highSchools, "([高(級)中學]$)")
regex_hsTitle <- "([高(級)中學])$"
...
```{r ans21}
first <- str_split(data228$`檔號`, "/")
pick_first <- map(first, ~ .x[1])
final_first <- factor(unlist(pick_first))

pick_second <- map(first, ~ .x[2])
final_second <- as.integer(unlist(pick_second))

data228[["全宗名編號"]] <- final_first
data228[["年"]] <- final_second
...
```{r ans22}
print(data228$`附註`)
table(data228$`附註`=="表冊名稱:臺灣省二二八事變正法及死亡人犯名冊")
pick <- (data228$`附註`=="表冊名稱:臺灣省二二八事變正法及死亡人犯名冊")
data228$`檔號`[pick]
table(data228$`檔號`=="A803000000A/0036/340.2/5502.3/19/007")
victims <- unique(data228$`個人名稱`[data228$`檔號` == (data228$`檔號`[pick])])
class(victims)
...
```{r ans31}
table(courseStructure$`應修系級`)
pick <- str_extract(courseStructure$`應修系級`, "[:graph:]+系[0-9]")
majorGradeCombos <- str_extract(courseStructure$`應修系級`, "[:graph:]+系[0-9]")
majorGradeCombos
```
