...
```{r ans12}
choose <- map(cpiByIncome, ~ .x$Item)
choose <- unlist(choose)
items <- factor(choose)
...
family_type <- items
levels(items)
new_level <- str_extract(levels(items), "[:graph:]+(?=#)")
levels(family_type) <- new_level
...
```{r ans14}
subcategory <- items
sub_level <- str_extract(levels(items), "(?<=#)[:graph:]+(?=\\()")
levels(subcategory) <- sub_level
