...
data <- list(
  type = ntpc_building_info$type[pick],
  floer = ntpc_building_info$floer[pick],
  name = ntpc_building_info$name[pick], 
  name_en = ntpc_building_info$name_en[pick]
)

ntpc_building_parking <- data.frame(data)
...
no_found <- function(key_word){
  pick <- str_detect(ntpc_building_info$name, key_word)
  if(sum(pick)==0){
    warning("No Found")
  }
}
...
```{r ans2}
best_choice <- function(cake, sweet){
  if((cake>bear[6,]$`蛋糕(Q1)`) | (sweet<bear[6,]$`甜甜圈(Q2)`)){
    return("少消費蛋糕、多消費甜甜圈")
  } else if((cake<bear[6,]$`蛋糕(Q1)`) | (sweet>bear[6,]$`甜甜圈(Q2)`)){
    return("多消費蛋糕、少消費甜甜圈")
  } else if((cake=bear[6,]$`蛋糕(Q1)`) & (sweet=bear[6,]$`甜甜圈(Q2)`)){
    return("維持現有消費組合")
  }
...
list_availableSeats <- function(seatingChart){
  for(i in 1:7){
  c <- paste0(rep((names(seatingChart)[i]),7), "-", seat)
  a <- c(a,c)}
  return(a)
}
class(list_availableSeats(seatingChart))
...
```{r ans33}
seat1 <- c(7, 5, 0, 3, 1, 2, 4, 0, 6)
update_seatingChart <- function(seatingChart, selectedSeat){
  select <- str_split(selectedSeat, "-")
  row <- select[[1]][[1]]
  col <- as.integer(select[[1]][[2]])
  if(seatingChart[[row]][col==seat1]==-1){
    warning("很抱歉您所選的位置已無法選")
  } else {
  seatingChart[[row]][col==seat1] <- -1
  return(seatingChart)
  }
}
...
assign_seats <- function(people, seatingChart){
  sample <- sample(list_availableSeats(seatingChart), length(people), replace = F)
  names(sample) <- people
  return(sample)
}
...
```{r ans35}
seat1 <- c(7, 5, 0, 3, 1, 2, 4, 0, 6)
remove_seats_covid19 <- function(seatingChart){
  sample <- sample(list_availableSeats(seatingChart), 1)
  select <- str_split(sample, "-")
  row <- select[[1]][[1]]
  col <- as.integer(select[[1]][[2]])
  if(seatingChart[[row]][col==seat1]==0 & (which(col==seat1))==9){
    seatingChart[[row]][col==seat1] <- -1
    seatingChart$row1[[(which(col==seat1)-1)]] <- -1
    return(seatingChart)
  } else if(seatingChart[[row]][col==seat1]==0 & (which(col==seat1))==1) {
    seatingChart[[row]][col==seat1] <- -1
    seatingChart$row1[[(which(col==seat1)+1)]] <- -1
    return(seatingChart)
  } else {
...
```{r ans36}
assign_seats_covid19 <- function(people, seatingChart){
  for(i in 1 : length(people)){
    sample <- remove_seats_covid19(seatingChart)
    names(sample) <- people[i]
    return(sample)
   if(i>15){
     break
    }  
